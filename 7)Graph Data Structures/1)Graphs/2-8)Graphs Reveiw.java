Graph Review
    ...
    For this exercise, we will build out the train network inside TrainNetwork.java.

Instructions
    1.
    In the .main() method, start by creating the train network as a weighted and directed Graph instance and assigning it to trainNetwork.
    ...
    We just got funding to build out 6 train stations. Using the graph’s .addVertex() method, add the following station vertices to our trainNetwork with the names:
        ...
        "Denver"
        "Calgary"
    Remember to save the resulting vertices in variables, which we will use later.

    3.
    We only want to service the routes our customers will travel the most, so let’s use .addEdge() to add the following route edges with the corresponding distances to the graph:
        From San Francisco to Los Angeles (400mi)
    ...
    Using the graph’s .removeEdge() and .removeVertex() methods, remove the route from New York to Denver, all the routes to and from Calgary, and the Calgary station itself.

    5.
    We’re finally up to date with our train stations. Print out the final graph and check that we built the following routes:
