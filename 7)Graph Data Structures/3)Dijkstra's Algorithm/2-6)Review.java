Review
    ...
    1)
    We "first" 'need to initialize the two objects' that we would use to 'keep track of the shortest paths from the starting vertex to every vertex in the graph'. <1> The previous object "keeps track of the preceding vertices in the path", like a 'reverse linked-list'. We can use it to "reconstruct the entire path", but backwards. <2> The distances object "keeps track of how far each vertex is from the starting vertex".
...
Instructions
    Now that we have created a graph let’s begin to tinker around with it.

    Here are a few ideas:
        : When you call dijkstraResultPrinter() from the main() method try passing in a different argument for the startingVertex parameter. Notice how the distance and previous values printed to the terminal change.

        : Do the same by passing in new arguments for startingVertex and targetVertex passed into the shortestPathBetween() method.

        : Try rearranging the edges and weights to see how the algorithm will react!